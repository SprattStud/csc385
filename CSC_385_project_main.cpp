        ...
        if (theFile.is_open())
        {
            if (getline(theFile, line))
			{
				...
				while(getline(theFile, firstLine))		// get the first line of the next user record if we're not at end of file
				{
					...
					getline(theFile, line);
                			theUser.password = theUser.passwordEncryptDecrypt(line);
    ...
    bool writeListToFile(string filename)
    {
        ...
        if (theFile.is_open())
        {
            ...
            for (int i = 0; i < list.size(); i++)
            {
                ...
                theFile << theUser.passwordEncyotDecrypt(theUser.password); << endl;
...
User menu_user_add()
{
    ...
    cout << "Password: ";
    getline(cin, editedUser.passwordEncryptDecrypt(editedUser.password));
